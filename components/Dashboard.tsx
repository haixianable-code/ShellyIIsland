  ...
  return (
    <div className="p-4 max-w-2xl mx-auto space-y-8 animate-fadeIn pb-2">
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-6">
        {canGetPack ? (
           <button onClick={() => setShowCrateModal(true)} className="bg-[#ffa600] p-4 md:px-6 md:py-8 rounded-[2rem] md:rounded-[2.5rem] shadow-[0_6px_0_#e65100] border-4 border-white flex flex-col items-center justify-center relative overflow-hidden group hover:bg-[#ffb74d] transition-all bubble-button h-full min-h-[160px]">
             <div className="bg-white/30 p-3 md:p-4 rounded-2xl shadow-sm mb-2 md:mb-3 border-2 border-white/50 z-10 animate-float">
      ...
      <div className="space-y-6 pt-2">
        <button onClick={() => { playClick(); onStartStudy(); }} disabled={newWordsAvailable === 0} className={`group w-full p-5 md:p-7 rounded-[2.5rem] md:rounded-[3rem] font-black flex items-center justify-between transition-all bubble-button ${newWordsAvailable > 0 ? "bg-[#ff7b72] text-white hover:bg-[#ff8a80] shadow-[0_8px_0_#d32f2f] border-4 border-white" : "bg-slate-100 text-slate-300 cursor-not-allowed shadow-none border-4 border-transparent"}`}>
          <div className="flex items-center space-x-4 md:space-x-5">
            <div className="bg-white/30 p-3 md:p-4 rounded-2xl shadow-inner border-2 border-white/40 group-hover:scale-110 transition-transform"><Shovel size={24} className={`fill-current md:w-8 md:h-8 ${newWordsAvailable > 0 ? 'animate-float-small' : ''}`} /></div>
