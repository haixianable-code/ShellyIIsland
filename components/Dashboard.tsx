  ...
  return (
    <div className="p-4 max-w-2xl mx-auto space-y-8 animate-fadeIn pb-2">
      <header className="text-center pt-6 md:pt-10 pb-4 relative z-10">
        <div className="inline-block bg-white border-[6px] md:border-[8px] border-[#4b7d78] rounded-[2.5rem] md:rounded-[3.5rem] px-6 py-6 md:px-8 md:py-10 shadow-[0_10px_0_rgba(0,0,0,0.1)] relative transform transition-transform hover:scale-105 duration-300 group">
            <h1 className="text-5xl md:text-8xl font-black text-[#4b7d78] tracking-tighter leading-none flex items-center justify-center gap-2 md:gap-5 flex-wrap">
                <span>SSI</span>
                <span className="text-[#ffa600]">{titleWord}</span>
                <span className="text-[#ff7b72]">!</span>
            </h1>
            <TreePalm className="absolute -left-8 -top-8 md:-left-12 md:-top-12 text-[#78c850] -rotate-12 drop-shadow-md" size={50} />
            <div className="absolute -right-5 -bottom-5 md:-right-8 md:-bottom-8 bg-[#ffeb3b] p-2 md:p-4 rounded-full border-[4px] border-[#fbc02d] animate-bounce shadow-md">
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-6">
        {canGetPack ? (
           <button onClick={() => setShowCrateModal(true)} className="bg-[#ffa600] p-4 md:px-6 md:py-8 rounded-[2rem] md:rounded-[2.5rem] shadow-[0_6px_0_#e65100] border-4 border-white flex flex-col items-center justify-center relative overflow-hidden group hover:bg-[#ffb74d] transition-all active:translate-y-2 bubble-button h-full min-h-[160px]">
          ...
          <div className={`p-4 md:px-6 md:py-8 rounded-[2rem] md:rounded-[2.5rem] shadow-[0_6px_0_rgba(0,0,0,0.1)] border-4 flex flex-col items-center justify-center relative overflow-hidden h-full min-h-[160px] ${isDailyGoalMet ? 'bg-[#3949ab] border-[#5c6bc0]' : 'bg-white border-[#f0f0f0]'}`}>
            <div className={`p-3 md:p-4 rounded-2xl shadow-sm mb-2 md:mb-3 border-2 z-10 ${isDailyGoalMet ? 'bg-white/20 border-white/30' : 'bg-[#ffcc80] border-white'}`}>
              {isDailyGoalMet ? <Moon className="text-[#e8eaf6] fill-current" size={24} /> : <Sprout className="text-white fill-current group-hover:scale-110 transition-transform" size={24} />}
            </div>
      ...
      <div className="space-y-6 pt-2">
        <button onClick={() => { playClick(); onStartStudy(); }} disabled={newWordsAvailable === 0} className={`group w-full p-5 md:p-7 rounded-[2.5rem] md:rounded-[3rem] font-black flex items-center justify-between transition-all active:translate-y-2 ${newWordsAvailable > 0 ? "bg-[#ff7b72] text-white hover:bg-[#ff8a80] shadow-[0_8px_0_#d32f2f] border-4 border-white" : "bg-slate-100 text-slate-300 cursor-not-allowed shadow-none border-4 border-transparent"}`}>
          <div className="flex items-center space-x-4 md:space-x-5">
            <div className="bg-white/30 p-3 md:p-4 rounded-2xl shadow-inner border-2 border-white/40"><Shovel size={24} className="fill-current md:w-8 md:h-8" /></div>
        ...
        {learnedToday.length > 0 && (
          <div className="bg-[#e8f5e9] p-5 md:p-6 rounded-[2.5rem] border-4 border-[#8bc34a] shadow-[0_6px_0_#689f38] relative overflow-hidden">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2"><span className="bg-white px-2 py-0.5 rounded-full text-[10px] font-black text-[#2e7d32] border border-[#8bc34a]">{learnedToday.length}</span><button onClick={(e) => { e.stopPropagation(); playClick(); onViewAllHarvest?.(); }} className="bg-white p-2 rounded-xl text-[#2e7d32] hover:bg-[#c8e6c9] transition-all shadow-sm border border-[#8bc34a] flex items-center justify-center"><List size={16} strokeWidth={3} /></button></div>
            </div>
        ...
        <button onClick={() => { playClick(); onStartReview(); }} disabled={wordsDue === 0} className={`w-full p-5 md:p-7 rounded-[2.5rem] md:rounded-[3rem] font-black flex items-center justify-between transition-all active:translate-y-2 ${wordsDue > 0 ? "bg-[#88d068] text-white hover:bg-[#96e072] shadow-[0_8px_0_#5a9a3b] border-4 border-white" : "bg-slate-100 text-slate-300 cursor-not-allowed border-4 border-transparent"}`}>
          <div className="flex items-center space-x-4 md:space-x-5">
            <div className="bg-white/30 p-3 md:p-4 rounded-2xl shadow-inner border-2 border-white/40"><GlassWater size={24} className={`md:w-8 md:h-8 ${wordsDue > 0 ? "animate-pulse" : ""}`} /></div>
