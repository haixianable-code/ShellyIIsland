...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const webAppJsonLd = {
    ...
    "@type": "WebApplication",
    "name": "Shelly Spanish Island",
    "description": "AI-powered Spanish vocabulary mastery with biological memory anchors.",
  ...
  const courseJsonLd = {
    "@context": "https://schema.org",
    "@type": "Course",
    "name": "Island Vocabulary Path",
    "description": "A structured Spanish curriculum based on RAE frequency data and AI mnemonics.",
    "provider": {
      "@type": "Organization",
      "name": "Shelly Spanish Island",
      "sameAs": "https://ssisland.space/"
    },
    "hasCourseInstance": VOCABULARY_DATA.map(pack => ({
      "@type": "CourseInstance",
      "name": pack.title,
      "description": `Master high-frequency Spanish words related to ${pack.title}.`
    }))
  };
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto space-y-6 animate-fadeIn pb-2 relative">
      ...
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden opacity-[0.05]" aria-hidden="true">
      ...
      <header className="text-center pt-4 md:pt-8 pb-2 relative z-10">
        <div className="inline-block bg-white border-4 md:border-[6px] border-[#4b7d78] rounded-[2rem] md:rounded-[3rem] px-6 py-5 md:px-10 md:py-8 shadow-[0_8px_0_rgba(0,0,0,0.1)] relative transform transition-transform hover:scale-105 duration-300 group">
            ...
            <TreePalm className="absolute -left-6 -top-6 md:-left-10 md:-top-10 text-[#78c850] -rotate-12 drop-shadow-md animate-float" size={40} aria-hidden="true" />
        </div>
      ...
      <section className="grid grid-cols-2 gap-3 md:gap-6 relative z-10" aria-label="Status Summary">
        {canGetPack ? (
           <button 
          ...
          <div className={`p-4 md:px-6 md:py-8 rounded-3xl shadow-[0_6px_0_rgba(0,0,0,0.05)] border-4 flex flex-col items-center justify-center relative overflow-hidden h-full min-h-[140px] ${isDailyGoalMet ? 'bg-[#3949ab] border-[#5c6bc0]' : 'bg-white border-[#f0f0f0]'}`}>
            <div className={`p-2.5 md:p-3.5 rounded-xl shadow-sm mb-2 border-2 z-10 ${isDailyGoalMet ? 'bg-white/20 border-white/30' : 'bg-[#ffcc80] border-white'}`} aria-hidden="true">
            ...
            <div className="text-center z-10">
              ...
              <h2 className={`block text-[8px] md:text-[9px] font-bold uppercase tracking-widest ${isDailyGoalMet ? 'text-[#c5cae9]' : 'text-[#8d99ae]'}`}>
        ...
        <div className="bg-white p-4 md:px-6 md:py-8 rounded-3xl shadow-[0_6px_0_#f0f0f0] border-4 border-[#f0f0f0] flex flex-col items-center justify-center relative h-full min-h-[140px]">
          <div className="bg-[#a5d6a7] p-2.5 md:p-3.5 rounded-xl shadow-sm mb-2 border-2 border-white z-10" aria-hidden="true">
          ...
          <h2 className="text-[8px] md:text-[9px] font-black uppercase text-[#8d99ae] tracking-widest z-10">{t('ui.dashboard.daily_care')}</h2>
        </div>
      </section>

      <section className="space-y-4 pt-2 relative z-10" aria-label="Learning Actions">
        <button 
          onClick={() => { playClick(); onStartStudy(); }} 
          disabled={newWordsAvailable === 0} 
          aria-label={newWordsAvailable > 0 ? `Start planting ${newWordsAvailable} new words` : "No new seeds available to plant"}
          className={`group w-full p-5 md:p-6 rounded-[2rem] md:rounded-[2.5rem] font-black flex items-center justify-between transition-all bubble-button ${newWordsAvailable > 0 ? "bg-[#ff7b72] text-white hover:bg-[#ff8a80] shadow-[0_8px_0_#d32f2f] border-2 md:border-4 border-white" : "bg-slate-100 text-slate-300 cursor-not-allowed shadow-none border-2 border-transparent"}`}
        >
          <div className="flex items-center space-x-4">
            <div className="bg-white/30 p-2.5 md:p-3 rounded-xl shadow-inner border-2 border-white/40" aria-hidden="true">
        ...
        <button 
          ...
          disabled={wordsDue === 0} 
          aria-label={wordsDue > 0 ? `Water garden and refresh ${wordsDue} words` : "Your garden is fully watered"}
          className={`w-full p-5 md:p-6 rounded-[2rem] md:rounded-[2.5rem] font-black flex items-center justify-between transition-all bubble-button group ${wordsDue > 0 ? "bg-[#88d068] text-white hover:bg-[#96e072] shadow-[0_8px_0_#5a9a3b] border-2 md:border-4 border-white" : "bg-slate-100 text-slate-300 cursor-not-allowed border-2 border-transparent"}`}
        >
          <div className="flex items-center space-x-4">
            <div className="bg-white/30 p-2.5 md:p-3 rounded-xl shadow-inner border-2 border-white/40" aria-hidden="true">
              <GlassWater size={20} className={`md:w-6 md:h-6 ${wordsDue > 0 ? "animate-float-small" : ""}`} />
            </div>
            <div className="text-left">
              <div className="text-lg md:text-xl">{t('ui.dashboard.water_garden')}</div>
              <div className="text-[8px] md:text-[9px] uppercase opacity-80 tracking-widest font-black">
                {t('ui.dashboard.refresh_grown', { count: wordsDue })}
              </div>
            </div>
          </div>
          <ArrowRight className="text-white w-5 h-5 md:w-6 md:h-6 group-hover:translate-x-1 transition-transform" strokeWidth={3} aria-hidden="true" />
      ...
      <aside 
        ...
        aria-label="Daily slang snippet"
      >
        <div className={`p-2 rounded-xl shadow-md border-2 shrink-0 bg-white ${isSpicy ? 'border-rose-300' : 'border-[#fdd835]'}`} aria-hidden="true">
          {isSpicy ? <Flame className="text-rose-500 fill-current w-5 h-5" /> : <Snail className="text-[#795548] fill-current w-5 h-5" />}
        </div>
        <div className={`text-xs font-bold leading-snug flex-1 transition-all duration-150 ${isRotating ? 'opacity-0 scale-95' : 'opacity-100 scale-100'} ${isSpicy ? 'text-rose-900' : 'text-[#5d4037]'}`}>
          <div className="flex items-center gap-2 mb-0.5">
            <strong className={`text-[10px] font-black uppercase tracking-widest block ${isSpicy ? 'text-rose-500' : 'text-[#e91e63]'}`}>{t('ui.dashboard.slang_title')}</strong>
            {isSpicy && <span className="bg-white/80 text-[7px] font-black text-rose-500 border border-rose-200 px-1 py-0.5 rounded uppercase tracking-wider">🔞 Spicy</span>}
          </div>
          <span className={`font-black text-base md:text-lg block leading-tight ${isSpicy ? 'text-rose-600' : 'text-[#d32f2f]'}`}>"{currentSlang.s}"</span>
          <p className="text-[10px] opacity-70 mb-0.5">{currentSlang.t}</p>
          <p className={`text-[8px] font-black uppercase tracking-wider ${isSpicy ? 'text-rose-800/60' : 'text-[#795548]/60'}`}>Lit: {currentSlang.note}</p>
        </div>
        <button 
          onClick={rotateSlang} 
          aria-label="Rotate to next slang"
          className={`bg-white p-2 rounded-xl border shadow-sm transition-all active:scale-90 ${isSpicy ? 'border-rose-300 text-rose-400' : 'border-[#fdd835] text-[#fbc02d]'}`}
        >
          <RotateCw size={14} className={isRotating ? 'animate-spin' : ''} aria-hidden="true" />
        </button>
      </aside>
