...
import { X, Zap, ArrowRightLeft, Sparkles, BookOpen, Map, Layers, Heart, Volume2, Star } from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-6 md:p-10">
          <div className="flex flex-col items-center">
            ...
            <div className="flex gap-2 mb-4">
              <div className={`px-4 py-1 rounded-full text-white font-black text-[10px] uppercase tracking-[0.2em] shadow-sm ${word.type === 'verb' ? 'bg-[#ff7043]' : word.type === 'adj' ? 'bg-[#fbc02d]' : word.type === 'misc' ? 'bg-[#ab47bc]' : 'bg-[#2196f3]'}`}>
                {word.type}
              </div>
              {word.reg === false && (
            ...
            {/* Main Word - Click to Speak */}
            <button 
              onClick={() => playAudio(word.s)}
              className="group active:scale-95 transition-transform"
              title="Tap to listen"
            >
              <h2 className="text-5xl md:text-6xl font-black text-[#2e4d4a] tracking-tighter mb-2 text-center flex items-center justify-center gap-3">
                {word.s}
                <Volume2 size={32} className="text-[#2e4d4a] opacity-0 group-hover:opacity-20 transition-opacity" />
              </h2>
            </button>
            ...
            <div className="w-full space-y-6">
              {/* Grammar Section - Special handling for Misc words */}
              {word.type === 'misc' ? (
                 <section className="bg-[#f3e5f5] p-6 rounded-[2.5rem] border-4 border-[#ce93d8] relative overflow-hidden shadow-sm">
                   <div className="flex items-center gap-2 mb-4">
                     <Star size={18} className="text-[#9c27b0]" fill="currentColor" />
                     <h4 className="text-[12px] font-black text-[#9c27b0] uppercase tracking-wider">Golden Rule</h4>
                   </div>
                   <p className="text-[#4a148c] font-bold text-lg leading-relaxed italic text-center px-4">
                     {word.grammarTip}
                   </p>
                 </section>
              ) : (
              ...
              <section className="bg-[#e3f2fd] p-8 rounded-[3rem] border-b-[8px] border-[#90caf9] relative overflow-hidden shadow-sm">
                ...
                <div className="space-y-6 mb-6">
                  {word.examples.map((ex, i) => (
                    <div 
                      key={i} 
                      className="pl-4 border-l-4 border-[#90caf9] cursor-pointer group active:opacity-60 transition-opacity"
                      onClick={() => playAudio(ex.txt)}
                    >
                      <div className="flex items-center gap-2">
                         <p className="text-[#1565c0] text-xl font-black italic leading-tight">"{ex.txt}"</p>
                         <Volume2 size={16} className="text-[#1565c0] opacity-0 group-hover:opacity-50 transition-opacity" />
                      </div>
