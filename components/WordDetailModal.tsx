...
import { X, Volume2, Sparkles, AudioLines, BookOpen, PenTool, BrainCircuit, Loader2, Send, MessageSquareText, Heart, Lock, Crown, Eye, ChevronDown, History, Sun, Rocket, Target, CheckCircle2, AlertTriangle, RefreshCcw, Shuffle, Feather, Lightbulb, Camera, Film } from 'lucide-react';
...
const WordDetailModal: React.FC<WordDetailModalProps> = ({ word, onClose }) => {
  ...
  const { aiCache, consumeAIToken, profile, openModal, aiUsage, mnemonicUsage } = useIslandStore();
  ...
  const renderStemMappedText = (conjugated: string) => {
    ...
    const base = word.s.toLowerCase().replace(/se$/, ''); 
    ...
    if (prefixLength < 2) return <span style={{ color: theme.main }} className="font-black underline decoration-2 underline-offset-2">{conjugated}</span>;
  ...
  // --- 时态过滤逻辑 (Fix: 区分 Snapshot 和 Movie) ---
  ...
  }, [word.examples, timeState, pastMode]);

  // --- 获取当前显示的变位列表 ---
  const getActiveForms = () => {
    if (timeState === 'future') return word.tense_forms?.future;
    if (timeState === 'past') {
        return pastMode === 'snapshot' ? word.tense_forms?.past : word.tense_forms?.imperfect;
    }
    return word.forms;
  };
  ...
  const fetchMission = async (count: number) => {
    ...
    if (mission) {
        setContext({ type: 'ai_mission', text: mission.task_text, subtext: mission.subtext, level: level });
    }
  ...
  const handleAiChallenge = async (e: React.FormEvent) => {
    ...
    setIsAiProcessing(true); setAiFeedback(null); playSwish();
    try {
  ...
  const headerBgColor = timeState === 'past' ? (pastMode === 'snapshot' ? '#795548' : '#5d4037') : timeState === 'future' ? '#2196f3' : theme.main;
  ...
  const handleSpeak = (e: React.MouseEvent, text: string) => {
    e.stopPropagation(); setActiveText(text);
      ...
      <div className="relative w-full max-w-md bg-[#fffdf5] rounded-[3rem] shadow-[0_30px_60px_-10px_rgba(0,0,0,0.3)] overflow-hidden animate-zoomIn flex flex-col max-h-[95vh]">
        <div className="overflow-y-auto no-scrollbar h-full relative">
            ...
            <div className="px-6 md:px-8 pt-16 pb-12 bg-[#fffdf5] relative z-10 space-y-8">
                {/* --- TIME MACHINE UI RE-INTEGRATED --- */}
                {hasTimeMachine && (
                    <div className="space-y-4">
                        <div className="flex items-center justify-center gap-2 mb-2 animate-slideUp">
                            <button onClick={() => { playShaker(); setTimeState('past'); }} className={`p-3 rounded-2xl border-2 transition-all ${timeState === 'past' ? 'bg-[#795548] text-white border-[#795548] shadow-md scale-110' : 'bg-white border-[#d7ccc8] text-[#a1887f] hover:bg-[#efebe9]'}`}><History size={20} /></button>
                            ...
                            <button onClick={() => { playClick(); setTimeState('present'); }} className={`p-3 rounded-2xl border-2 transition-all ${timeState === 'present' ? 'bg-[#ffa600] text-white border-[#ffa600] shadow-md scale-110' : 'bg-white border-[#ffe0b2] text-[#ffcc80] hover:bg-[#fff3e0]'}`}><Sun size={20} /></button>
                            <div className="h-1.5 w-16 bg-slate-200 rounded-full relative overflow-hidden"><div className={`absolute top-0 bottom-0 w-1/3 rounded-full transition-all duration-300 ${timeState === 'past' ? 'left-0 bg-[#795548]' : timeState === 'future' ? 'left-2/3 bg-[#2196f3]' : 'left-1/3 bg-[#ffa600]'}`} /></div>
                            <button onClick={() => { playSwish(); setTimeState('future'); }} className={`p-3 rounded-2xl border-2 transition-all ${timeState === 'future' ? 'bg-[#2196f3] text-white border-[#2196f3] shadow-md scale-110' : 'bg-white border-[#bbdefb] text-[#90caf9] hover:bg-[#e3f2fd]'}`}><Rocket size={20} /></button>
                        </div>

                        {timeState === 'past' && hasImperfect && (
                            <div className="flex justify-center animate-slideDown">
                                <div className="bg-[#efebe9] p-1 rounded-xl flex gap-1 border border-[#d7ccc8]">
                                    <button onClick={() => { playClick(); setPastMode('snapshot'); }} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${pastMode === 'snapshot' ? 'bg-[#795548] text-white shadow-sm' : 'text-[#8d6e63] hover:bg-[#d7ccc8]/50'}`}>
                                        <Camera size={12} /> Fui (Snapshot)
                                    </button>
                                    <button onClick={() => { playClick(); setPastMode('movie'); }} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${pastMode === 'movie' ? 'bg-[#5d4037] text-white shadow-sm' : 'text-[#8d6e63] hover:bg-[#d7ccc8]/50'}`}>
                ...
                <div className="flex items-center gap-3">
                   <button onClick={() => { playClick(); setActiveTool(activeTool === 'mnemonic' ? 'none' : 'mnemonic'); }} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl border-2 transition-all active:scale-95 ${activeTool === 'mnemonic' ? 'bg-[#f3e5f5] border-[#ab47bc] text-[#6a1b9a]' : 'bg-white border-slate-100 text-slate-400 hover:border-[#ab47bc]/30'}`}><BrainCircuit size={18} /><span className="text-[10px] font-black uppercase tracking-widest">{t('ui.ai.tool_mnemonic')}</span>{activeTool === 'mnemonic' && <ChevronDown size={14} />}</button>
                   <button onClick={() => { playClick(); setActiveTool(activeTool === 'challenge' ? 'none' : 'challenge'); }} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl border-2 transition-all active:scale-95 ${activeTool === 'challenge' ? 'bg-[#e1f5fe] border-[#0288d1] text-[#0288d1]' : 'bg-white border-slate-100 text-slate-400 hover:border-[#0288d1]/30'}`}><Sparkles size={18} /><span className="text-[10px] font-black uppercase tracking-widest">{t('ui.ai.tool_challenge')}</span>{activeTool === 'challenge' && <ChevronDown size={14} />}</button>
                </div>
                    ...
                    {activeTool === 'challenge' && (
                        <div className="bg-white p-6 rounded-[2.5rem] border-4 border-[#e1f5fe] shadow-sm relative overflow-hidden transition-all">
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-2"><MessageSquareText size={16} className="text-[#0288d1]" /><span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#4b7d78]">{context?.level ? `Stage ${context.level} Lab` : t('ui.ai.challenge_title')}</span></div>
                            </div>
                            <div className={`p-3 rounded-2xl border mb-3 flex items-center justify-between group transition-colors ${context?.level === 2 ? 'bg-[#fff8e1] border-[#ffecb3]' : 'bg-[#f0f9ff] border-[#b3e5fc]'}`}>
                                <div className="flex items-center gap-3">
                                    <div className={`p-1.5 rounded-lg text-white ${context?.level === 2 ? 'bg-[#f57c00]' : 'bg-[#0288d1]'}`}>{isMissionLoading ? <Loader2 size={14} className="animate-spin" /> : <Target size={14} />}</div>
                                    <div className="flex-1"><p className="text-[9px] font-black uppercase tracking-widest opacity-70">Mission Protocol</p><p className="text-xs font-bold text-[#4b7d78] leading-tight">{isMissionLoading ? "Synthesizing goal..." : context?.text}</p>{context?.subtext && !isMissionLoading && <p className="text-[10px] text-slate-400 mt-1 italic leading-tight">{context.subtext}</p>}</div>
                                </div>
                                <button onClick={(e) => { e.stopPropagation(); playSwish(); setRefreshCount(r => r + 1); fetchMission(refreshCount + 1); }} className="p-2 rounded-xl text-[#0288d1] hover:bg-white transition-all active:scale-90 active:rotate-180 duration-300"><Shuffle size={16} /></button>
                            </div>
                            <form onSubmit={handleAiChallenge} className="relative z-10 space-y-3">
                                <div className="bg-slate-50 rounded-2xl p-4 border-2 border-dashed border-slate-200 focus-within:border-[#0288d1] transition-colors"><textarea value={userInput} onChange={(e) => setUserInput(e.target.value)} placeholder="Fix the sentence or construct yours..." className="w-full bg-transparent text-sm font-bold text-[#4b7d78] focus:outline-none placeholder:text-slate-300 min-h-[60px]" /></div>
                                <button type="submit" disabled={!userInput.trim() || isAiProcessing || isMissionLoading} className="w-full bg-[#0288d1] text-white py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-[0_4px_0_#01579b] hover:bg-[#03a9f4] active:translate-y-1 disabled:opacity-30 transition-all flex items-center justify-center gap-2">{isAiProcessing ? <Loader2 className="animate-spin" /> : <><Send size={14} /> {t('ui.ai.check')}</>}</button>
                            </form>
                            {aiFeedback && (
                                <div className={`mt-6 p-5 rounded-2xl border-2 animate-zoomIn ${aiFeedback.score >= 80 ? 'bg-[#e8f5e9] border-[#8bc34a]' : 'bg-[#fff3e0] border-[#ffa600]'}`}>
                                    <div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2">{aiFeedback.score >= 80 ? <CheckCircle2 size={16} className="text-[#2e7d32]" /> : <AlertTriangle size={16} className="text-[#e65100]" />}<span className={`text-[10px] font-black uppercase tracking-widest ${aiFeedback.score >= 80 ? 'text-[#2e7d32]' : 'text-[#e65100]'}`}>{aiFeedback.compliment}</span></div><span className="text-xs font-black bg-white px-2 py-0.5 rounded-md border border-slate-100 shadow-sm">{aiFeedback.score}/100</span></div>
                                    <div className="space-y-2"><div><p className="text-[9px] font-black uppercase text-slate-400 mb-0.5">{context?.level === 3 ? 'Expert Recast' : 'Correction'}</p><p className="text-sm font-bold text-[#2d4a47] italic">"{aiFeedback.correction}"</p></div><div className="pt-2 border-t border-black/5"><p className="text-[9px] font-black uppercase text-slate-400 mb-0.5">Tutor Analysis</p><p className="text-xs font-bold text-[#4b7d78] leading-snug">{aiFeedback.explanation}</p></div></div>
                                </div>
                            )}
                        </div>
                    )}
                        ...
                        <div className={`bg-gradient-to-br from-[#f3e5f5] to-[#e1f5fe] p-6 rounded-[2rem] border-4 border-white shadow-md relative overflow-hidden group transition-all`}>
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    {!isPremium && <div className="text-[9px] font-black text-[#8d99ae] bg-white/50 px-2 py-0.5 rounded-full border border-white">Daily: {aiUsage.count}/3</div>}
                                </div>
                                {!isMnemonicRevealed ? (
                                    ...
                                    <div className="space-y-4 animate-fadeIn">
                                        {isMnemonicLoading ? <div className="flex items-center justify-center py-8 text-[#ab47bc]"><Loader2 className="animate-spin" size={32} /></div> : (
                                            <><div><p className="text-[8px] font-black text-[#ab47bc] uppercase tracking-widest mb-1">{t('ui.ai.mnemonic_label')}</p><p className="text-[#2d4a47] font-bold text-sm leading-tight">"{aiInfo?.mnemonics || 'Thinking...'}"</p></div><div className="pt-3 border-t border-white/40"><p className="text-[8px] font-black text-[#0288d1] uppercase tracking-widest mb-1">{t('ui.ai.hint_label')}</p><p className="text-[#2d4a47] text-xs font-medium leading-relaxed">{aiInfo?.hint || 'Analyzing...'}</p></div></>
                                        )}
                                    </div>
                                )}
                            </div>
                                ...
                                {pronouns.map((p, i) => {
                                    ...
                                    return (
                                        <button key={p} onClick={(e) => { e.stopPropagation(); playAudio(formText); try { playClick(); } catch(err) {} }} className="flex items-center justify-between border-b border-slate-200/60 pb-1 last:border-0 hover:bg-white hover:shadow-sm hover:px-2 hover:-mx-2 hover:rounded-lg transition-all active:scale-95 cursor-pointer group w-full">
                                            ...
                                            <div className="flex items-center gap-1.5"><span className="text-sm font-black text-[#2d4a47]">{renderStemMappedText(formText)}</span><Volume2 size={10} className="text-[#2d4a47] opacity-0 group-hover:opacity-30 transition-opacity" /></div>
                                        </button>
                                    );
                                })}
                            </div>
                ...
                <div className="space-y-6">
                    <div className="flex items-center gap-2 mb-2 opacity-40 px-2"><BookOpen size={14} /><span className="text-[9px] font-black uppercase tracking-widest">{t('ui.study.usage_examples')}</span>{timeState !== 'present' && <span className="ml-auto text-[8px] font-black bg-slate-100 px-2 py-0.5 rounded-full uppercase">{timeState === 'past' ? (pastMode === 'snapshot' ? 'Past' : 'Imperfect') : timeState}</span>}</div>
                    {finalExamples.map((ex, i) => (
                        <button key={i} onClick={(e) => handleSpeak(e, ex.txt)} className="w-full text-left bg-white p-5 rounded-[2rem] shadow-sm border border-slate-100 hover:border-slate-200 hover:shadow-md transition-all active:scale-[0.98] group animate-fadeIn"><p className="text-lg font-black text-[#2d4a47] italic leading-tight mb-2 flex items-start gap-1 tracking-tight"><span className="text-slate-300 -ml-2">“</span>{ex.txt}</p><p className="text-xs font-bold text-slate-400 pl-3 border-l-2 border-slate-200 leading-tight">{ex.eng}</p></button>
                    ))}
                </div>
            </div>
        </div>
